...
# ==============================================================================
# QUANTIZATION (INT6 PACKED + ZSTD/ZLIB)
# ==============================================================================
...
def quantize_state_dict_int8(flat_state: dict[str, mx.array]) -> tuple[dict[str, object], dict[str, int]]:
    ...
    for name, arr in flat_state.items():
        ...
        if q.ndim == 2 and s.ndim > 0:
            ...
            stats["int6_payload_bytes"] += int(packed_data.nbytes + s.nbytes)
        else:
            quantized[name] = q
...
def main() -> None:
    ...
    compress_name = "zstd" if _HAS_ZSTD else "zlib"
    log(
        f"serialized_model_int6_{compress_name}:{quant_file_bytes} bytes "
        f"(payload:{quant_stats['int6_payload_bytes']} raw_pickle:{quant_serialized_bytes} payload_ratio:{ratio:.2f}x)"
    )

    with quant_path.open("rb") as f:
        quant_blob_disk = f.read()
    quant_flat = dequantize_state_dict_int8(pickle.loads(decompress_bytes(quant_blob_disk)))
