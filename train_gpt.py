...
def eval_val_sliding_window(
    ...
    # Score each token with a nearly full context window instead of resetting context
    # at every non-overlapping validation block.
    seq_len = args.train_seq_len
    total_tokens = val_tokens.numel() - 1
    window_starts = [
        start
        for start in range(0, total_tokens, stride)
        if min(start + seq_len, total_tokens) - start >= 1
    ]
    ...
    batch_seqs = int(os.environ.get("SW_EVAL_BATCH", 32))
    ...
    with torch.inference_mode():
        for batch_start in range(0, len(my_windows), batch_seqs):
            ...
            for i, start in enumerate(batch_windows):
                ...
                score_start = 0 if start == 0 else max(window_len - stride, 0)
...
# Instead, we quantize large tensors to packed int6 and compress the serialized state.
...
def pack_int6(q: Tensor) -> tuple[Tensor, int, list[int]]:
    """Pack signed int6 values stored in an int8 tensor into 3 bytes per 4 values."""
    shape = list(q.shape)
    flat = q.detach().to(device="cpu", dtype=torch.int16).reshape(-1)
    count = int(flat.numel())
    pad = (-count) % 4
    if pad:
        flat = torch.cat((flat, torch.zeros(pad, dtype=torch.int16)))
    u = (flat + 32).clamp_(0, 63).view(-1, 4)
    b0 = ((u[:, 0] << 2) | (u[:, 1] >> 4)).to(torch.uint8)
    b1 = (((u[:, 1] & 0x0F) << 4) | (u[:, 2] >> 2)).to(torch.uint8)
    b2 = (((u[:, 2] & 0x03) << 6) | u[:, 3]).to(torch.uint8)
    return torch.stack((b0, b1, b2), dim=1).reshape(-1).contiguous(), count, shape

def unpack_int6(packed: Tensor, count: int, shape: list[int]) -> Tensor:
    """Unpack a uint8 tensor produced by pack_int6 back to an int8 tensor."""
    b = packed.detach().to(device="cpu", dtype=torch.int16).reshape(-1, 3)
    v0 = (b[:, 0] >> 2) & 0x3F
    v1 = ((b[:, 0] & 0x03) << 4) | (b[:, 1] >> 4)
    v2 = ((b[:, 1] & 0x0F) << 2) | (b[:, 2] >> 6)
    v3 = b[:, 2] & 0x3F
    flat = torch.stack((v0, v1, v2, v3), dim=1).reshape(-1)[:count]
    return (flat - 32).to(torch.int8).reshape(shape).contiguous()
...
def quantize_state_dict_int8(state_dict: dict[str, Tensor]):
    ...
    # - packed per-row int6 for 2D float tensors
    # - packed per-tensor int6 for other large float tensors
    ...
    for name, tensor in state_dict.items():
        ...
        packed_q, orig_count, orig_shape = pack_int6(q)
        qmeta[name] = {
            "scheme": "int6_packed",
        ...
        stats["int6_payload_bytes"] += tensor_nbytes(packed_q) + tensor_nbytes(s)
...
def dequantize_state_dict_int8(obj: dict[str, object]) -> dict[str, Tensor]:
    ...
    for name, q_raw in obj["quantized"].items():
        dtype = getattr(torch, obj["dtypes"][name])
        s = obj["scales"][name]
        meta = qmeta.get(name, {})
        if meta.get("scheme") == "int6_packed":
            q = unpack_int6(q_raw, int(meta["orig_count"]), list(meta["orig_shape"]))
        else:
            q = q_raw
        if meta.get("axis") == 0 or s.ndim > 0:
...
def run_mlx_fallback() -> None:
    if "RANK" in os.environ or "WORLD_SIZE" in os.environ:
        raise RuntimeError("CUDA is required for distributed train_gpt.py runs")
    mlx_script = Path(__file__).with_name("train_gpt_mlx.py")
    if not mlx_script.is_file():
        raise RuntimeError("CUDA is required and train_gpt_mlx.py was not found for local fallback")
    print("CUDA unavailable; launching train_gpt_mlx.py for local MLX training.", flush=True)
    os.execv(sys.executable, [sys.executable, str(mlx_script), *sys.argv[1:]])
...
def main() -> None:
    ...
    # the compressed int6 artifact and validate the round-tripped weights.
    ...
    if master_process:
        ...
        compress_name = "zstd" if HAS_ZSTD else "zlib"
        log0(
    ...
    if args.eval_stride > 0 and args.eval_stride < args.train_seq_len:
        ...
        torch.cuda.synchronize()
        log0(
            f"final_int6_sliding_window stride:{args.eval_stride} "
